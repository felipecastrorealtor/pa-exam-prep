...
// Design tokens
// ─────────────────────────────────────────────────────────────────────────────
const A  = '#4f8ef7'   // accent blue
const A2 = '#7c5cfc'   // accent violet
...
// ─────────────────────────────────────────────────────────────────────────────
// Feature icons — thin-stroke SVG family
// ─────────────────────────────────────────────────────────────────────────────
...
const UNITS_ES = [
  ...
  'Derechos e Intereses de Propiedad',
  'Gravámenes, Embargos y Servidumbres',
  'Transferencia de Título y Escrituras',
  'Contratos y Agencia',
  'Financiamiento e Hipotecas',
  'Arrendamientos y Gestión de Propiedades',
  'Valuación y Tasación',
  'Uso del Suelo y Zonificación',
  'Leyes de Vivienda Justa',
  'Ley de Licencias de PA',
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#0a0d14', color: '#f1f5f9' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            ...
            <button
              ...
              style={{
                ...
                color: A, letterSpacing: '0.06em', transition: 'background .15s',
              }}
            ...
            <Link href="/login" className="lnav-link"
              style={{ fontSize: '0.875rem', color: '#94a3b8', textDecoration: 'none', transition: 'color .15s' }}>
              {t.navSignIn}
            </Link>
            <Link href="/register" className="lbtn-primary"
              style={{
                fontSize: '0.875rem', background: A, color: '#fff',
                padding: '7px 18px', borderRadius: 9, fontWeight: 700,
      ...
      <section style={{ maxWidth: 896, margin: '0 auto', padding: '96px 20px 80px', textAlign: 'center' }}>
        ...
        <p style={{
          fontSize: 'clamp(.98rem, 2vw, 1.18rem)', color: '#94a3b8',
          maxWidth: 600, margin: '0 auto 36px', lineHeight: 1.72,
        }}>
          {t.heroSub}
        </p>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 14, justifyContent: 'center', marginBottom: 20 }}>
          <Link href="/register" className="lbtn-primary"
            style={{
              background: A, color: '#fff', fontSize: '1rem', fontWeight: 700,
              padding: '14px 34px', borderRadius: 14, textDecoration: 'none',
              boxShadow: '0 8px 28px rgba(79,142,247,.32)', transition: 'background .15s',
            }}>
            {t.heroCta}
          </Link>
          <Link href="/login" className="lbtn-ghost"
            style={{
              border: '1px solid rgba(255,255,255,.14)', color: '#cbd5e1',
              fontSize: '1rem', fontWeight: 600, padding: '14px 34px',
              borderRadius: 14, textDecoration: 'none', transition: 'border-color .15s',
            }}>
            {t.heroSignIn}
          </Link>
        </div>
      ...
      <section style={{
        ...
        <div style={{
          maxWidth: 896, margin: '0 auto', padding: '32px 20px',
          display: 'grid', gridTemplateColumns: 'repeat(3,1fr)',
        }}>
          {([
            { val: '321', label: t.statsQ },
            { val: '22',  label: t.statsU },
            { val: '100%', label: t.statsB },
      ...
      <section style={{ maxWidth: 1152, margin: '0 auto', padding: '96px 20px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(280px,1fr))', gap: 20 }}>
          {t.features.map(({ icon, title, desc }) => {
            ...
            return (
              <div key={icon} className="lcard" style={{
                background: '#111520',
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(230px,1fr))', gap: 10 }}>
            {t.units.map((unit, i) => (
              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 10, fontSize: '0.875rem', color: '#94a3b8' }}>
                <span style={{
                  ...
                  borderRadius: 6, color: A, fontSize: '0.68rem',
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, fontSize: '0.875rem', color: '#475569' }}>
              <span style={{
                ...
                borderRadius: 6, color: '#475569', fontSize: '0.75rem',
                display: 'flex', alignItems: 'center', justifyContent: 'center',
              }}>+</span>
              {t.unitsMore}
            </div>
      ...
      <section style={{ maxWidth: 896, margin: '0 auto', padding: '96px 20px 48px' }}>
        <div style={{ textAlign: 'center', marginBottom: 44 }}>
          <h2 style={{ fontSize: 'clamp(1.7rem,4vw,2.5rem)', fontWeight: 800, color: '#fff', letterSpacing: '-0.02em', marginBottom: 12 }}>
            {t.pricingHeading}
          </h2>
        ...
        <div style={{
          ...
          gap: 20, maxWidth: 640, margin: '0 auto 32px',
        }}>
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              {t.freeFeatures.map((f) => (
                <div key={f} style={{ display: 'flex', alignItems: 'center', gap: 10, fontSize: '0.875rem', color: '#94a3b8' }}>
                  <IconCheck /> {f}
                </div>
              ))}
            </div>
            <Link href="/register" className="lbtn-ghost"
              style={{
                display: 'block', textAlign: 'center',
                border: '1px solid rgba(255,255,255,.15)', color: '#f1f5f9',
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              {t.paidFeatures.map((f) => (
                <div key={f} style={{ display: 'flex', alignItems: 'center', gap: 10, fontSize: '0.875rem', color: 'rgba(255,255,255,.9)' }}>
                  <IconCheck light /> {f}
                </div>
              ))}
            </div>
            <Link href="/register" className="lbtn-white"
              style={{
                display: 'block', textAlign: 'center',
                background: '#fff', color: A2,
                fontWeight: 700, padding: '12px 24px',
                borderRadius: 12, textDecoration: 'none', transition: 'background .15s',
              }}>
              {t.paidCta}
            </Link>
          </div>
        ...
        {/* Access code — no "for 30 days free" */}
        <p style={{ textAlign: 'center', fontSize: '0.875rem', color: '#475569' }}>
          {t.accessCode}{' '}
          <Link href="/register"
            style={{ color: A, textDecoration: 'none', fontWeight: 500 }}>
            {t.accessCodeLink}
          </Link>.
        </p>
      </section>
        ...
        <div style={{ maxWidth: 600, margin: '0 auto', padding: '80px 20px', textAlign: 'center' }}>
          ...
          <Link href="/register" className="lbtn-primary"
            style={{
              ...
              background: A, color: '#fff', fontWeight: 700, fontSize: '1.05rem',
